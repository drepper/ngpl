...
fn test_generate_curried():
  let arr := generate(multiply(3), 1…5)
  assert_eq(5, arr.sizeof)
  assert_eq(3, arr[0])
  assert_eq(6, arr[1])
  assert_eq(9, arr[2])
...
fn test_generate_stepped():
  let arr := generate(λx : int → int: x, 0…2…10)
...
fn test_generate_capture():
  let offset := 100
  let arr := generate(λx : int |offset| → int: x + offset, 1…3)
  assert_eq(3, arr.sizeof)
  assert_eq(101, arr[0])
  assert_eq(102, arr[1])
  assert_eq(103, arr[2])
...
fn test_generate_single():
  let arr := generate(λx : int → int: x, 5…5)
...
fn test_generate_slice():
  let arr := generate(λx : int → int: x, 1…10)
  let sub := arr[2…4]
...
fn test_range_value():
  let r := 1…5
  let arr := generate(λx : int → int: x + 1, r)
  assert_eq(5, arr.sizeof)
  assert_eq(2, arr[0])
  assert_eq(6, arr[4])
