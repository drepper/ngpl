...
fn test_generate_basic:
  var arr = generate(λx : int: x * 2, 1…5)
...
fn test_generate_stepped:
  var arr = generate(λx : int: x, 0…2…10)
...
fn test_generate_descending:
  var arr = generate(λx : int: x * x, 3…1)
...
fn test_generate_foreach:
  var arr = generate(λx : int: x * 10, 1…3)
  var sum = 0
  foreach v = arr:
    sum ← sum + v
  assert_eq(60, sum)
...
fn test_generate_slice:
  var arr = generate(λx : int: x, 1…10)
  var sub = arr[2…4]
...
fn test_generate_not_range:
  generate(λx : int: x, 42)

// Range as first-class value
@test
fn test_range_value:
  var r = 1…5
  var arr = generate(λx : int: x + 1, r)
  assert_eq(5, arr.sizeof)
  assert_eq(2, arr[0])
  assert_eq(6, arr[4])
