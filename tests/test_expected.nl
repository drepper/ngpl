...
@test
fn test_optional_postfix_some -> none:
    var r := opt_double(some(5))
    assert_eq(r ?? -1, 10)

@test
fn test_optional_postfix_none -> none:
    var r := opt_double(none)
    assert_eq(r ?? -1, -1)

/* ---- division returns expected value ------------------------------------- */

@test
fn test_div_success_unwrap -> none:
    var x := 10 / 3
    var v := x ?? -1
    assert_eq(v, 3)

@test
fn test_div_zero_recovery -> none:
    var x := 10 / 0
    var v := x ?? -1
    assert_eq(v, -1)

@test
fn test_mod_success_unwrap -> none:
    var x := 10 % 3
    var v := x ?? -1
    assert_eq(v, 1)

@test
fn test_mod_zero_recovery -> none:
    var x := 10 % 0
    var v := x ?? -1
    assert_eq(v, -1)
...
@test
fn test_expected_propagate_ok -> none:
    var r := safe_div(10, 2)
    assert_eq(r ?? -1, 5)

@test
fn test_expected_propagate_err -> none:
    var r := safe_div(10, 0)
    assert_eq(r ?? -1, -1)
...
@test
fn test_div_chain_recovery -> none:
    var a := (100 / 10) ?? 0
    var b := (a / 2) ?? 0
    assert_eq(b, 5)

@test
fn test_div_chain_zero -> none:
    var a := (100 / 0) ?? 0
    var b := (a / 2) ?? 0
    assert_eq(b, 0)
...
@test
fn test_expected_err_is_std_errors -> none:
...
@test
fn test_unwrapped_expected_arithmetic -> none:
    var x := (20 / 4) ?? 0
    var y := x + 10
    assert_eq(y, 15)
...
@test
fn test_bang_return_ok -> none:
    var r := bang_div(10, 2)
    assert_eq(r ?? -1, 5)

@test
fn test_bang_return_err -> none:
    var r := bang_div(10, 0)
    assert_eq(r ?? -1, -1)
...
@test
fn test_bang_param_ok -> none:
    var v := 10 / 2
    assert_eq(bang_param(v), 5)

@test
fn test_bang_param_err -> none:
    var v := 10 / 0
