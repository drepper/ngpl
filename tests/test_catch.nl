...
fn test_catch_oob_expected():
    let result := safe_get_result([1, 2, 3], 10)
    let val := result ?? ⁻1
    assert_eq(val, ⁻1)
...
fn test_catch_expected_success():
    let result := safe_get_result([1, 2, 3], 1)
    let val := result ?? ⁻1
    assert_eq(val, 2)
...
fn test_catch_multi_stmt_success():
    let result := multi_stmt([10, 20, 30], 1)
    let val := result ?? ⁻1
    assert_eq(val, 21)

@test
fn test_catch_multi_stmt_fail():
    let result := multi_stmt([10, 20, 30], 5)
    assert_eq(result, ∅)
...
fn test_catch_returns_value_on_success():
    let result := catch_then_continue([5, 6, 7], 0)
    let val := result ?? ⁻1
    assert_eq(val, 5)
