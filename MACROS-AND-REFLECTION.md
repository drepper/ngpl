...
| Branch | Design | Suite |
|--------|--------|-------|
| `macros-rules` | a macro is a list of rewrite rules | 87 test files, 101 output tests, `-Werror` clean |
| `macros-proc` | a macro is a function over the program's text | 87 test files, 102 output tests, `-Werror` clean |
...
- **Design A** fits the majority of macros anyone writes, which are
  shorthand — `swap⟦x, y⟧`, `assert_eq⟦a, b⟧`, `unwrap⟦e⟧`.  Each is a
  shape and a replacement, and writing it as a function is ceremony
  around a rewrite.
- **Design B** fits the minority that must *look* at what they were
